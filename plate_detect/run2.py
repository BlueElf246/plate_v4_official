...
def plate1(img, show=False):
    ...
    plt.imshow(binary_car_image*255, cmap='gray')
    plt.show()
    ...
    for region in regionprops(label_image):
        ...
        if region_height >= min_height and region_height <= max_height and region_width >= min_width and region_width <= max_width and region_width > region_height:
            ...
            cv2.rectangle(img, (min_col, min_row), (max_col, max_row), (255,0,0,), thickness=3)
            ax1.add_patch(rectBorder)
            # let's draw a red rectangle over those regions
    if show==True:
        plt.show()
    return plate_objects_cordinates, img
def detect_plate1(img, show=False):
    if type(img)==str:
        ...
        for x,path_img in enumerate(img_list):
            ...
            plate_objects_cordinates, img_car = plate1(i, show=show)
            for z, coor in enumerate(plate_objects_cordinates):
                ...
                cv2.imwrite(f'/Users/datle/Desktop/plate_v4/img_car_with_plate/result_{x}_{z}.png', img=img_car)

    else:
        plate_objects_cordinates, img_car= plate1(img, show=show)
        p=[]
        for z,coor in enumerate(plate_objects_cordinates):
            img1=img[coor[0]:coor[2],coor[1]:coor[3]]
            p.append(img1)
            cv2.imwrite(f'/Users/datle/Desktop/plate_v4/result_plate/result_{z}.png', img=img1)
            cv2.imwrite(f'/Users/datle/Desktop/plate_v4/img_car_with_plate/result_{z}.png', img=img_car)
    return p
